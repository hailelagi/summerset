...
usage() {
    cat <<'EOF'
Run a distributed TLC master (tlc2.tool.distributed.TLCServer).

Usage:
  ./tla+/tlc_distr_master.sh -s <SPEC_DIR> -n <SPEC_NAME> [OPTIONS]

Options:
  -s, --spec-dir <DIR>        spec folder under tla+/ (required)
  -n, --spec-name <NAME>      e.g. MultiPaxos (required)
  -c, --cfg-suffix <SUFFIX>   config suffix, e.g. small
      --hostname <HOST>       value for -Djava.rmi.server.hostname (default: $(hostname -f))
      --fpmem-ratio <F>       TLC -fpmem value, 0<F<=1 (default 0.5)
      --num-fpsets <K>        block startup until K fpset shards register (default 0)
...
Workers connect to this host on RMI port 10997; the jar download server
listens on 10996. Both ports must be reachable from worker hosts.
EOF
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --num-fpsets)     NUM_FPSETS="$2"; shift 2 ;;
...
if [[ "$NUM_FPSETS" -gt 0 ]]; then
    jvm_props+=(-Dtlc2.tool.distributed.TLCServer.expectedFPSetCount="$NUM_FPSETS")
fi

echo "TLC master host: ${RMI_HOST}  (RMI 10997, HTTP 10996)"
echo "Expected fpset shards: ${NUM_FPSETS}"
...
exec java -XX:+UseParallelGC "-Xmx${XMX}" \
     "${jvm_props[@]}" \
     -cp "$JAR_PATH" tlc2.tool.distributed.TLCServer \
     -fpmem "$FPMEM_RATIO" \
     -metadir "$STATES_DIR" \
     -config "${SPEC_NAME}_MC${cfg_tag}" \
     "${SPEC_NAME}_MC"
